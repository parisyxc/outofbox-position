...
Modeled on a public track-record site's information architecture, reduced to what your agent actually produces.
That model's moat is **transparency + a scored record**; copy that, not the 10-product sprawl.

## Navigation (top bar)
`Home · Position · Daily · Screener · Method · Record · Reports · About · [X] [Substack] · Join`
+ a scrolling **ticker tape of currently-tracked names with P&L since entry** (a market tape, but only names you own the record for).

## Pages

| Route | Reference analogue | Content source | What it shows |
|---|---|---|---|
...
| `/position` | Position / record | `data/ledger.json` | **Open book** (ticker, added date/price, plan, score, stop, target, P&L) + **closed history** (entry/exit/reason). Gold = entry disclosed, white = exit disclosed. |
| `/daily` · `/daily/YYYY-MM-DD` | Structure Read / posts | `reports/*/*.md` | Archive of daily reports; each day = the ParisYoung's-Position-style post |
...
| `/about` · `/join` | About / Join | static | Who, why, disclaimer; optional subscription later ($/mo) |
...
## Design notes (from the reference site)
